...
class Evaluator():
    ...
    def load_data(self):
        ...
        x_test_fgsm = np.load('../../OtherAttacks/generated_data/fgsm/mnist/x_test_adv.npy')
        logging.info('[evaluate]: FGSM generated data is loaded.')

        # PGD generated data
        # x_test_pgd = np.load('attacks_data/PGD/x_test_pgd_step10.npy')
        x_test_pgd = np.load(
            '../../OtherAttacks/generated_data/pgd_step{0}/mnist/x_test_adv.npy'.format(self.config ['attack_pgd_step']))
    ...
    def evaluate(self, model_type):
        ...
        if model_type == 'original':
            ...
            logging.info('[evaluate]: ({0} step-PGD-robust model on IGN generated test data): {1}'.format(get_config()['defense_pgd_step'], pgd_robust_on_ign_test_acc))
