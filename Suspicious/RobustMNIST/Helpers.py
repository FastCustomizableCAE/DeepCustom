...
def suspicious_fmap_inds():
    inds = []
    # read configuration file
    with open('config.json') as config_file:
        config = json.load(config_file)
    load_name = 'suspicious_detection/suspicious_feature_maps/suspicious_feature_maps_{0}.pkl'.format(config['class'])
    with open(load_name, 'rb') as input:
        suspicious_feature_maps = pickle.load(input)
    model = load_model('mnist_5x30')
    num_conv_layers, _ = calculate_layer_types(model= model)
    index_dictionary = {}
    for i in range(num_conv_layers):
        index_dictionary[get_convolutional_layer_index(model= model, order= i)] = i
        inds.append([])
    for fmap in suspicious_feature_maps:
        layer = fmap.get_layer()
        depth = fmap.get_depth()
        inds[index_dictionary[layer]].append([depth])
    print(inds)
    return inds

def normal_fmaps_inds():
    # read configuration file
    with open('config.json') as config_file:
        config = json.load(config_file)
    model = load_model('mnist_5x30')
    ...
    with open(load_name, 'rb') as input:
        suspicious_feature_maps = pickle.load(input)
    normal_feature_map_inds, inds = [], []
    index_dictionary = {}
    for i in range(num_conv_layers):
        index_dictionary[get_convolutional_layer_index(model=model, order=i)] = i
        layer_index = get_convolutional_layer_index(model= model, order= i)
        _, _, depth = get_layer_properties(model= model, layer= layer_index)
        normal_feature_map_inds.append(list(range(depth)))
    for fmap in suspicious_feature_maps:
        order = index_dictionary[fmap.get_layer()]
        normal_feature_map_inds[order].remove(fmap.get_depth())
    for i in range(len(normal_feature_map_inds)):
        normal_feature_map_inds[i] = list(map(lambda x: [x], normal_feature_map_inds[i]))
    print(normal_feature_map_inds)
    return normal_feature_map_inds

# for Dense Layers
def suspicious_neurons():
    # read configuration file
    with open('config.json') as config_file:
        config = json.load(config_file)
    model = load_model('mnist_5x30')
    load_name = 'suspicious_detection/suspicious_dense_neurons/suspicious_dense_neurons_{0}.pkl'.format(config['class'])
    with open(load_name, 'rb') as input:
        suspicious_neurons = pickle.load(input)
    _, num_dense_layers = calculate_layer_types(model=model)
    suspicious_neuron_indices = []
    for neuron in suspicious_neurons:
        layer_no = neuron.get_layer()
        neuron_no = neuron.get_neuron_no()

        #print('layer no: {0}, neuron no: {1}'.format(layer_no, neuron_no))

        neuron_index = get_dense_neuron_index(model= model, layer_no= layer_no, neuron_no= neuron_no)

        suspicious_neuron_indices.append(int(neuron_index))
    return suspicious_neuron_indices
...
def normal_neurons():
    ...
    with open(load_name, 'rb') as input:
        suspicious_neurons = pickle.load(input)
    normal_neuron_indices = list(range(total_number_of_dense_neurons(model= model)))
    #print('normal_neuron_indices: ', normal_neuron_indices)
    for suspicious_neuron in suspicious_neurons:
        layer_no = suspicious_neuron.get_layer()
        neuron_no = suspicious_neuron.get_neuron_no()
        index = get_dense_neuron_index(model= model, layer_no= layer_no, neuron_no= neuron_no)
        #print('index to remove: ', index)
        normal_neuron_indices.remove(index)
    return normal_neuron_indices
...
def get_layer_properties(model, layer):
    ...
    activations = get_activations(model= model, sample= np.zeros((1, 784)))
    if layer_type == 'Conv2D':
