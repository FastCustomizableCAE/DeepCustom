...
class Evaluator():
    ...
    def load_data(self):
        ...
        # FGSM generated data
        x_test_fgsm = np.load('{0}/OtherAttacks/generated_data/fgsm/cifar/x_test_adv.npy'.format(os.getcwd()))
        logging.info('[evaluate]: FGSM generated data is loaded.')

        # PGD generated data
        x_test_pgd = np.load(
            '{0}/OtherAttacks/generated_data/pgd_step{1}/cifar/x_test_adv.npy'.format(os.getcwd(),self.config ['attack_pgd_step']))
        logging.info('[evaluate]: PGD generated data is loaded for step size: {0}'.format(self.config ['attack_pgd_step']))
        os.chdir('{0}/Suspicious/RobustCIFAR'.format(os.getcwd()))
    ...
    def evaluate(self, model_type):
        ...
        if model_type == 'original':
            ...
            logging.info('[evaluate]: (PGD-robust model on DGN generated test data): {0}'.format(pgd_robust_on_dgn_test_acc))
