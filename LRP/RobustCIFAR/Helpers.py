...
'''
def data_for_class(class_):
    X_train, y_train, X_test, y_test = load_CIFAR()
    print('Original Shapes: X_train: {0}, X_test: {1}'.format(X_train.shape, X_test.shape))
    x_train = X_train.astype('float32')
    x_test = X_test.astype('float32')
    x_train /= 255
    x_test /= 255
    train_label = list(map(lambda x: np.argmax(x), y_train))
    test_label =  list(map(lambda x: np.argmax(x), y_test))
    class_train_inds = np.where(np.array(train_label) == class_)
    class_test_inds = np.where(np.array(test_label) == class_)
    # print(np.array(train_label)[class_train_inds].shape)
    # print(np.array(test_label)[class_test_inds].shape)
    # print('class_train_inds shape: {0}'.format(class_train_inds[0].shape))
    # print('class_test_inds shape: {0}'.format(class_test_inds[0].shape))
    return x_train[class_train_inds[0]], y_train[class_train_inds[0]], x_test[class_test_inds[0]], y_test[class_test_inds[0]]
'''
