...
def custom_loss(original_image, generated_image, class_, name='lrp_custom_loss'):
    ...
    with tf.op_scope([original_image, generated_image, class_], name, 'lrp_custom_loss') as scope:
        ...
        # Relevant pixels
        relevant_pixels_original = gather_cols(original_image, relevant_pixels(for_class= class_, k= k))
        relevant_pixels_generated = gather_cols(generated_image, relevant_pixels(for_class= class_, k= k))

        # Non-relevant pixels
        non_relevant_pixels_original = gather_cols(original_image, non_relevant_pixels(for_class= class_, k= k))
        non_relevant_pixels_generated = gather_cols(generated_image, non_relevant_pixels(for_class= class_, k= k))

        # MSE of normal pixels  MSE(org + eps, gen)
