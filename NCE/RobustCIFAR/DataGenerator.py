...
class DataGenerator():
    ...
    def generate_data_for_all_classes(self):
        ...
        for class_ in range(self.num_classes):
            ...
            # ensure pixel values are in between 0 and 1
            np.clip(generated_data, 0.0, 1.0, out=generated_data)
            all_generated_data[class_indices] = generated_data.reshape((len(generated_data), 32, 32, 3))
    ...
    def check_mis_classification(self, original_data, generated_data, target):
        ...
        generated_data = generated_data.reshape((len(generated_data), 32, 32, 3))
    ...
    def generate_data_using_DGN(self, class_, x):
        ...
        if self.clipping:
            clipped_generated_data = np.zeros((len(generated_data), 3072))
            for index in range(len(generated_data)):
                clipped_data = clip_pixels(original_image=x[index].reshape(3072, ),
                            generated_image=generated_data[index].reshape(3072, ),
                            eps=self.eps)
