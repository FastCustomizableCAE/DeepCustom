...
This script evaluates DGN networks against each other.
...
def load_dgn_robust_network(project_path):
    if not os.path.exists('{0}/dgnRobustModel'.format(project_path)):
        raise FileNotFoundError()
    else:
        config = tf.ConfigProto()
        sess = tf.Session(config=config)
        graph = tf.get_default_graph()
        saver = tf.train.import_meta_graph('{0}/dgnRobustModel/dgn_robust_model.meta'.format(project_path))
        saver.restore(sess, tf.train.latest_checkpoint('{0}/dgnRobustModel'.format(project_path)))
        # get tensors
...
def dgn_robust_model_evaluate(x, y, project_path):
    sess, X, X_adv, Y, acc = load_dgn_robust_network(project_path)
...
if __name__ == '__main__':
    ...
    for attack_metric in attack_metrics:
        ...
        robust_test_acc = dgn_robust_model_evaluate(x=generated_test_data, y=y_test, project_path=defense_project_path)
        print('Robust test accuracy of {0}-DGN-network on {1}-DGN-attack is: {2}'.format(defense_metric, attack_metric,
                                                                                         robust_test_acc))
